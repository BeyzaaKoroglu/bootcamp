...
from baskets.serializers import BasketItemSerializer, BasketSerializer, BasketItemDetailedSerializer, \
    BasketDetailedSerializer, BasketItemValidateSerializer
from core.mixins import DetailedViewSetMixin


class BasketViewSet(DetailedViewSetMixin, mixins.RetrieveModelMixin, mixins.ListModelMixin, GenericViewSet):
    permission_classes = ()
    ...
    def list(self, request, *args, **kwargs):
        ...
        if not user_id:
              create_serializer = BasketSerializer(data={"customer": None, "status": BasketStatus.OPEN})
        ...
        if not basket:
            create_serializer = BasketSerializer(data={"customer": user_id, "status": BasketStatus.OPEN})
            create_serializer.is_valid(raise_exception=True)
            basket = create_serializer.save()
            serializer = self.get_serializer(basket)
            return Response(serializer.data, status=status.HTTP_200_OK)
    ...
    def basket_item(self, *args, **kwargs):
        ...
        if quantity > 0:
            item_serializer = BasketItemSerializer(instance=basket_item, data={
                "basket": basket.id, "product": product.id, "quantity": quantity, "price": product.price.amount
            })
            item_serializer.is_valid(raise_exception=True)
            item_serializer.save()
        elif basket_item:
