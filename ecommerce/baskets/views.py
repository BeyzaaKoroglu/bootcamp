...
class BasketViewSet(DetailedViewSetMixin, mixins.RetrieveModelMixin,
    ...
    def get_queryset(self):
        ...
        if user_id:
            loggined_basket = queryset.filter(customer_id=user_id, status=BasketStatus.OPEN).first()
            lookup_url_kwarg = self.lookup_url_kwarg or self.lookup_field
            if lookup_url_kwarg in self.kwargs:
                filter_kwargs = {
                    self.lookup_field: self.kwargs[lookup_url_kwarg]
                }  # {"slug": "5667880-23423sdfsd"}
                anonymous_basket = queryset.filter(**filter_kwargs).exclude(customer_id=user_id).first()
                if loggined_basket and anonymous_basket:
                    with atomic():
                        ...
                        for old_basket_item in loggined_basket.basketitem_set.all():
                            new_basket_item = anonymous_basket.basketitem_set.filter(product_id=old_basket_item.product_id).first()
                            quantity = old_basket_item.quantity
                            if new_basket_item:
                                quantity += new_basket_item.quantity
                            item_serializer = BasketItemSerializer(
                                instance=new_basket_item, data={"basket": anonymous_basket.id,
                                                                "product": old_basket_item.product_id,
                                                                "quantity": quantity,
                                                                "price": old_basket_item.price.amount})
